...
use regex::Regex;
use serde_generate::{deno, test_utils, CodeGeneratorConfig, Encoding, SourceInstaller};
use std::{fs::File, path::Path, process::Command};
use tempfile::tempdir;

fn test_that_deno_code_compiles_with_config(
    dir_path: &Path,
...
#[test]
fn test_that_deno_code_compiles_with_bcs() {
    let dir = tempdir().unwrap();
    let config =
        CodeGeneratorConfig::new("testing".to_string()).with_encodings(vec![Encoding::Bcs]);
    test_that_deno_code_compiles_with_config(&dir.path(), &config);
}

#[test]
fn test_that_deno_code_compiles_with_comments() {
    let dir = tempdir().unwrap();
    ...
    let path = test_that_deno_code_compiles_with_config(&dir.path(), &config);
